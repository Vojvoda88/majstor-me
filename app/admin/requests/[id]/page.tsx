...
export default async function AdminRequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    req = await prisma.request.findUnique({
      where: { id },
      select: ADMIN_REQUEST_DETAIL_SELECT,
    });
  } catch (err) {
    logAdminSsrFatal("[AdminRequestDetailSSR]", "request.findUnique", snapshot, err);
    const e = err instanceof Error ? err : new Error(String(err));
    return (
      <AdminRouteLoadError
        routeTitle="Zahtjev"
        cardTitle="Ne možemo učitati detalj zahtjeva"
        logPrefix="[AdminRequestDetailSSR]"
        message={e.message}
        code={prismaErrorCode(err)}
        snapshot={snapshot}
        resetHref="/admin/requests"
        resetLabel="Nazad na listu zahtjeva"
      />
    );
  }
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <p>
                <strong>Ime:</strong> {requesterName}
              </p>
              <p>
                <strong>Telefon:</strong> {req.requesterPhone ?? req.user?.phone ?? "-"}
              </p>
              <p>
                <strong>Email:</strong> {req.requesterEmail ?? req.user?.email ?? "-"}
              </p>
              <p>
                <strong>Grad:</strong> {req.city}
              </p>
              <p>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              <p>
                ...
                <Badge variant="outline">{ADMIN_STATUS_LABELS[req.adminStatus ?? ""] ?? req.adminStatus ?? "–"}</Badge>
              </p>
              <p>
                <strong>Status:</strong> <Badge>{STATUS_LABELS[req.status] ?? req.status}</Badge>
              </p>
              <p>
                <strong>Datum:</strong> {new Date(req.createdAt).toLocaleString("sr")}
              </p>
              <p>
                <strong>Broj ponuda:</strong> {req.offers.length}
              </p>
              <p>
                <strong>Broj otključanja:</strong> {req.contactUnlocks.length}
              </p>
            </CardContent>
          </Card>
        </div>
